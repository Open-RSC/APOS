...
public class B_Smithy extends Script {
	...
	private final int VROOOOM = 10; // Might need increasing to ~200. 10 is stupid fast.. But menus don't appear, and that's fun.
	...
	public void init(String s) {
		...
		int option = JOptionPane.showOptionDialog(null, "What type of bars would you like to use?", "Blood's Smithy", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE, null, bar_names, bar_names[5]);
		...
		panel.add(comboBox);
		option = JOptionPane.showConfirmDialog(null, panel, "Item selection", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
	...
	public int openDoors() {
		...
		boolean special;
		special = getY() < 507 ? isReachable(BANK_DOOR[1], BANK_DOOR[2] - 1) : isReachable(BANK_DOOR[1], BANK_DOOR[2]);
		if (bank_door == BANK_DOOR[0] && special) {
			atObject(BANK_DOOR[1], BANK_DOOR[2]);
			return random(800, 1200);
		}
		special = getY() < 510 ? isReachable(SMITHY_DOOR[1], SMITHY_DOOR[2] - 1) : isReachable(SMITHY_DOOR[1], SMITHY_DOOR[2]);
	...
	public void paint() {
		...
		y += 15;
		drawString(item.getItemName() + " made: " + items + " (" + perHour(items) + "/h)", x, y, 1, 0xFFFFFF);
		y += 15;
	...
	private int screenshot(long now) {
		...
		if (screenshot[2] == 1L) { // If Paint was enabled before the screenshot, turn it back on
			setPaintOverlay(!isPaintOverlay());
		}
		if (screenshot[2] == 1L) { // If Paint was enabled before the screenshot, turn it back on
