...
// TODO Auto-generated constructor stub
	...
	int fightMode;
	int[] spotToBankX = {311, 308, 304, 302, 305, 311, 312, 311, 308, 299, 311, 314, 313, 303, 294, 290, 287};
	int[] spotToBankY = {414, 423, 435, 447, 459, 467, 478, 488, 496, 506, 514, 526, 537, 543, 552, 566, 571};
	int[] bankToLumX = {287, 290, 290, 290, 279, 265, 251, 236, 222, 209, 194, 180, 166, 154, 139, 135, 134, 123};
	int[] bankToLumY = {571, 576, 589, 599, 609, 610, 610, 610, 607, 607, 604, 606, 611, 614, 616, 626, 636, 645};
	boolean inBankingSequence = false;
	int[] lootIds = {464, 220, 438, 439, 440, 441, 442, 443, 526, 527, 157, 158, 159, 160, 1277, 815, 817, 819, 821,
	...
	public void init(String params) {
		...
		fightModeSelected = (String) JOptionPane.showInputDialog(null, "Fight mode", "Wiz", JOptionPane.PLAIN_MESSAGE,
			null, fightModeOptions, fightModeOptions[0]);
		...
		bankEveryMinutes = Integer
			.parseInt(JOptionPane.showInputDialog("Always bank every minutes: (set -1 to disable)"));
...
// this handles fighting
		if (inArea(getX(), getY(), areaX, areaY, areaX2, areaY2) && getInventoryCount() != 30
			&& hasInventoryItem(foodid) && hasInventoryItem(1263) && !bankingTime()) {
...
// this handles walking to bank & banking & deathwalk.
		} else if ((/* getInventoryCount() == 30 && */!hasInventoryItem(1263) || !hasInventoryItem(foodid))
			|| (inBankingSequence == true) || getY() >= 574 || bankingTime()) {
			...
			if (!inArea(getX(), getY(), bankFrontX, bankFrontY, bankFrontX2, bankFrontY2) && getY() <= 574) {
...
// this handles walking to the wiz
		} else if (!inArea(getX(), getY(), areaX, areaY, areaX2, areaY2) && getInventoryCount() != 30
			&& !bankingTime()) {
	...
	private int[] _getReachableNpc(int... ids) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (inArray(ids, getNpcId(i))) {
				...
				if (!isReachable(x, y))
					continue;
				if (!inArea(x, y, areaX, areaY, areaX2, areaY2))
					continue;
