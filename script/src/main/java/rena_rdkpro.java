public class rena_rdkpro extends Script
{
    ...
    // Collect d bones, rlong, addypl8, runes, halfkeys, med sqs, herbs?, rune spear, addy bar
    int[] CollectIDs = {1092,1276,1277,795,75,814,526,527,120,619,42,38,31,33,442,441,174};
	...
	int walkBackPositionX = 146;
	int walkBackPositionY = 202;
    ...
    public rena_rdkpro (Extension e)
    {
        super(e);
    }
    ...
    public void init(String params) 
    {
        ...
        this.walkBack = Integer.parseInt(pa[17]);
        ...
        if(walkBack > 0)
        {
			if(walkBack == 1)
			{
				walkBackPositionX = alternateX1;
				walkBackPositionY = alternateY1;
			}
			else if(walkBack == 2)
			{
				walkBackPositionX = alternateX2;
				walkBackPositionY = alternateY2;
			}
			else if(walkBack == 3)
			{
				walkBackPositionX = alternateX3;
				walkBackPositionY = alternateY3;
			}
        }
	    ...
	    bankingnow = false;
		fightingnow = false;
        System.out.println("Abyte0 2021-08-24 Added Correct Drag Sq Id");
        System.out.println("Abyte0 2021-10-08 Added Walkback variants 0-3");
        System.out.println("Abyte0 2022-04-15 Stay calm while waiting");
        System.out.println("Abyte0 2022-10-07 Pickup addy bars");
		
		if(getX() >= 212 && getX() <= 220 && getY() >= 448 && getY() <= 453)
		{
			System.out.println("Starting from bank");
			bankingnow = true;
		}
		else
		{
			System.out.println("Starting from dragons");
			fightingnow = true;
		}
    }    
    ...
    public int main()
    {
        ...
        //Death walk
        if(walk_lumbydeath && !bankingnow && !recharge && !walk_edgetodragons && !fightingnow && !walk_dragonstoedge)
        {
            ...
            {
                step = 0;
                path = new int[] {138, 615, 157, 613, 173, 609, 185,605 ,192, 604, 193, 586, 193, 569, 190, 548, 205, 532, 217, 509, 217, 489, 220, 475, 215, 451};
            }
        ...
        if(!walk_lumbydeath && bankingnow && !recharge && !walk_edgetodragons && !fightingnow && !walk_dragonstoedge)
        {
            ...
            if(isBanking())
 
              {
                ...
                if(getInventoryCount(addyBarId) > 0)
                {            
                    addyBar  += getInventoryCount(addyBarId);
                    deposit(addyBarId, getInventoryCount(addyBarId));    
                    return random(900,1000);
                }
                ...
                System.out.println("Obtained "+addyBar+" rune longswords so far!");
        ...
        if(!walk_lumbydeath && !bankingnow && recharge && !walk_edgetodragons && !fightingnow && !walk_dragonstoedge)
        {
            if(getCurrentLevel(3) <= d || getHpPercent() < 80) 
            {
                //Eating
        ...
        if(!walk_lumbydeath && !bankingnow && !recharge && walk_edgetodragons && !fightingnow && !walk_dragonstoedge)
        {
            ...
            {
                step = 0;
                path = new int[] {208,446,191,435,190,425,191,415,190,405,191,395,191,385,191,375,191,365,191,355,191,345,191,335,191,325,191,315,191,305,191,295,191,285,191,275,191,265,191,255,191,245,191,235,191,225,191,215,191,205,191,195,191,185,191,175,171,175,161,175,151,175,141,179};
        ...
        if(!walk_lumbydeath && !bankingnow && !walk_edgetodragons && fightingnow && !walk_dragonstoedge)
        {        
            if(isAtApproxCoords(120,648,4))
            {
                walk_lumbydeath = true;
            ...
            // Drink Super Attack
            if(attack && getInventoryCount(486) > 0 || getInventoryCount(487) > 0 || getInventoryCount(488) > 0&& getCurrentLevel(0) <= j ) 
            {
                if(inCombat())
                {
                       walkTo(getX(), getY());
            ...
            // Drink Super Str
            if(str && getInventoryCount(494) > 0 || getInventoryCount(493) > 0 || getInventoryCount(492) > 0 && getCurrentLevel(2) <= m ) 
            {
                if(inCombat())
                {
                       walkTo(getX(), getY());
            ...
            if(bb2[0]!=-1)
            {
                ...
                walkTo(bb2[1], bb2[2]);
                pickupItem(1277, bb2[1], bb2[2]);
                System.out.println("[Rena] ZOMG! A DSQUARE DROP!!!"); 
                System.out.println("[Rena] Picking up half D-square at "+bb2[1]+","+bb2[2]+" ..."); 
                System.out.println(" "); 
                return random(200,300);
            ...
            int[] adamBar = getItemById(addyBarId);
            if(adamBar[0]!=-1)
            {
                if(inCombat())
                {
                ...
                walkTo(adamBar[1], adamBar[2]);
                pickupItem(addyBarId, adamBar[1], adamBar[2]);
                //System.out.println("[Rena] Picking up rune longsword at "+adamBar[1]+","+adamBar[2]+" ..."); 
                //System.out.println(" "); 
                return random(200,300);
            ...
            {
				if(getHpPercent() <= 78 && getInventoryCount(b) == 0)
				{
					ReturnToBank();
					return 500;
                }				
				...
				if(getX() != walkBackPositionX || getY() != walkBackPositionY)
					walkTo(walkBackPositionX,walkBackPositionY);
            ...
            if(!isAtApproxCoords(walkBackPositionX,walkBackPositionY,20) && !inCombat()) 
            {
                walkTo(walkBackPositionX,walkBackPositionY);//153,191
        ...
        if(!walk_lumbydeath && !bankingnow && !recharge && !walk_edgetodragons && !fightingnow && walk_dragonstoedge)
        {
            ...
            {
                step = 0;
                path = new int[] {141,179,151,175,161,175,171,175,191,175,191,185,191,195,191,205,191,215,191,225,191,235,191,245,191,255,191,265,191,275,191,285,191,295,191,305,191,315,191,325,191,335,191,345,191,355,191,365,191,375,191,385,191,395,190,405,191,415,190,425,191,435,208,446,218,446,216,451};
		...
		if((step + 1) < path.length)
		{
			...
			if(isAtApproxCoords(path[step], path[step + 1], 1))
				step = step + 2;
