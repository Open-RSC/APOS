...
public class T_HerbGather extends Script {
	...
	int loop = 0;
	int i = 0;
	int[] herbs = {438, 439, 440, 441, 442, 443, 526, 527, 157, 158, 159, 160, 1277, 815, 817, 819, 821, 823, 933, 40, 20, 33, 42};
	...
	public void init(String params) 
	{
        ...
        int option = JOptionPane.showOptionDialog(null, "Choose your weapon", "Toichi's Herb Gatherer", JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[3]);
	...
	public int main() 
	{
		...
		//System.out.println("problem!");
			...
			for(int h = 0; h < herbs.length; h++) 
			{
				...
				if(groundHerbs[0] != -1&&groundHerbs[1]>=344-5&&groundHerbs[1]<=344+5&&groundHerbs[2]>=3318-5&&groundHerbs[2]<=3318+5 ) 
				{
    ...
    public void paint() 
    {
        ...
        y += 15;
        drawString("Cadantine: " + gathered[4] + " (" + perHour(gathered[4]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
        drawString("Dwarf Weed: " + gathered[5] + " (" + perHour(gathered[5]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
        drawString("Key Half 1: " + gathered[6] + " (" + perHour(gathered[6]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
        drawString("Key Half 2: " + gathered[7] + " (" + perHour(gathered[7]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
        drawString("Uncut Diamond: " + gathered[8] + " (" + perHour(gathered[8]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
        drawString("Uncut Ruby: " + gathered[9] + " (" + perHour(gathered[9]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
        drawString("Uncut Emerald: " + gathered[10] + " (" + perHour(gathered[10]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
        drawString("Uncut Sapphire: " + gathered[11] + " (" + perHour(gathered[11]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
        drawString("Left Shield Half: " + gathered[12] + " (" + perHour(gathered[12]) + "/h)", x, y, 1, 0xFFFFFF);
        y += 15;
