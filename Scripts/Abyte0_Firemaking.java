...
//2022-04-10 Removed attacking blocking npc as they arent blocking on OpenRsc
...
public class Abyte0_Firemaking extends Abyte0_Script
{
	...
	private boolean killNpc = false;

	public Abyte0_Firemaking(Extension e)
	{
		super(e);
	}
	...
	public int main()
	{
		...
		if(_lastActionReturn == 0)
		{
			...
			if(_lastActionReturn != -1)
				return _lastActionReturn;
		}
		...
		if(_lastTree == null)
			_lastTree = findNextTree(myX,myY);
		...
		{
			////looking if npc in my way (not applicable to OpenRsc)
			//if(killNpc)
			//{
			//	int[] idNpcs = {0,4,8};
			//	for(int i = 0; i < 3; i++)
