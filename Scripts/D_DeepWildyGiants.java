...
// TODO Auto-generated constructor stub
	...
	int fightMode;
	int limpsBanked = 0;
	int[] lumbToP2PGateX = {131, 128, 117, 110, 113, 113, 110, 109, 107, 103, 95, 92, 84, 73, 66, 64, 64, 68, 70, 72, 73, 79, 88, 97, 106, 110, 110, 110, 109, 109, 110, 109, 110, 109, 111, 113, 117, 120, 119, 128, 130, 132, 130, 124, 122, 119, 121, 119, 118, 118, 118, 118, 117, 114, 111, 111, 111, 112, 108, 105, 106, 108, 105, 102, 111,};
	int[] lumbToP2PGateY = {637, 626, 625, 618, 610, 602, 595, 592, 587, 582, 579, 574, 571, 569, 564, 558, 551, 545, 539, 531, 521, 512, 508, 507, 507, 499, 490, 481, 475, 466, 456, 446, 436, 426, 419, 411, 405, 394, 384, 376, 369, 363, 355, 346, 339, 328, 321, 312, 304, 297, 288, 281, 273, 262, 254, 243, 233, 222, 210, 199, 190, 178, 167, 151, 143,};
	int[] gateToMBx = {113, 118, 129, 140, 152, 162, 175, 185, 193, 205, 214, 220, 227,};
	int[] gateToMBy = {132, 126, 121, 118, 115, 110, 102, 100, 99, 98, 98, 98, 105,};
	int[] mbtoSpotX = {227, 239, 247, 253, 262, 269,};
	int[] mbtoSpotY = {105, 110, 118, 126, 125, 127,};
	boolean inBankingSequence = false;
	int[] lootIds = {220, 413, 526, 527, 1277, 1276, 31, 32, 33, 34, 35, 36, 38, 40, 41, 46, 173, 73, 126, 448, 449, 450, 451, 452, 453, 157, 158, 159, 160, 10, 193, 411, 438, 439, 440, 441, 442, 443, 466, 40, 42,};
	...
	public void init(String params) {
		...
		fightModeSelected = (String) JOptionPane.showInputDialog(null, "Fight mode", "Thugs",
			JOptionPane.PLAIN_MESSAGE, null, fightModeOptions, fightModeOptions[0]);
		...
		if (answer == JOptionPane.YES_OPTION) {
			lootLimps = true;
			lootIds = new int[]{220, 413, 526, 527, 1277, 1276, 31, 32, 33, 34, 35, 36, 38, 40, 41, 46, 173, 73, 414, 126, 448, 449, 450, 451, 452, 453, 157, 158, 159, 160, 10, 193, 411, 438, 439, 440, 441, 442, 443, 466, 40, 42};
		} else {
			lootLimps = false;
		}
		...
		bankEveryMinutes = Integer.parseInt(JOptionPane.showInputDialog("Always bank every minutes: (set -1 to disable)"));
	...
	public boolean bankingTime() {
		...
		return (System.currentTimeMillis() - lastBankTimeMark) >= (bankEveryMinutes * 60000);
	}

	public int minutesToNextBank() {
		return (int) (((bankEveryMinutes * 60000) - (System.currentTimeMillis() - lastBankTimeMark)) / 60000);
	}
	...
	public int main() {
		...
		if (getFatigue() >= 96 && hasInventoryItem(1263) && !inCombat() && getCurrentLevel(3) >= autoEatHP) {
...
//this handles fighting
		if (isAtApproxCoords(268, 2963, 40) && getInventoryCount() != 30 && hasInventoryItem(foodid) && hasInventoryItem(1263) && !bankingTime()) {
...
//this handles walking to bank & banking & deathwalk.
		} else if (((!hasInventoryItem(1263) || !hasInventoryItem(foodid)) || (inBankingSequence == true) || bankingTime()) && getX() > 219) {
			...
			if (isAtApproxCoords(227, 109, 0) || isAtApproxCoords(227, 110, 0) || isAtApproxCoords(226, 110, 0)) {
		...
		} else if (!inArea(getX(), getY(), areaX, areaY, areaX2, areaY2) && getInventoryCount() != 30 && getX() > 219) {
	...
	public void paint() {
		...
		drawString(
			"Combat XP gained: "
				+ (((getAccurateXpForLevel(2) + getAccurateXpForLevel(1) + getAccurateXpForLevel(0)
				+ getAccurateXpForLevel(3)) - startxp)
				+ ", per hour: "
				+ (int) (((((getAccurateXpForLevel(2) + getAccurateXpForLevel(1)
				+ getAccurateXpForLevel(0) + getAccurateXpForLevel(3)) - startxp) * 60L) * 60L)
				/ time)),
			x, y, 1, 0xFFFFFF);
		y += 15;
		drawString("Prayer XP gained: " + (((getAccurateXpForLevel(5)) - startxpPray) + ", per hour: " + (int) (((((getAccurateXpForLevel(5)) - startxpPray) * 60L) * 60L) / time)),
			x, y, 1, 0xFFFFFF);
		y += 15;
		if (bankEveryMinutes != -1) {
			y += 15;
...
// areaXx,areaYy where first pair of coordinates represents the top left (north-western) corner & second set represents the bottom right (south-eastern) corner
...
//no tile within 25 dist, what the hell, lets walk if its even reachable.
	...
	// walks to furthest tile considering the max distance to the tile in reverse order, put tiles x & y in the matching
// indexes
