...
public class Abyte0_Sinister extends Abyte0_Script
{
	...
	public Abyte0_Sinister(Extension e) {		super(e);	}
	public void init(String params)
	{
		...
		if(!params.equals("default"))
		{
			...
			String choiceS = (String)JOptionPane.showInputDialog(frameS,		"Items:\n", "Spell Selection",		JOptionPane.PLAIN_MESSAGE, null, spellName, null);
			...
			String choiceRunesCount = (String)JOptionPane.showInputDialog(frameSpellCount,		"Items:\n", "Runes Count Selection",		JOptionPane.PLAIN_MESSAGE, null, runesCountName, null);
			...
			String choiceFoodCount = (String)JOptionPane.showInputDialog(frameFoodCount,		"Items:\n", "Food Count Selection",		JOptionPane.PLAIN_MESSAGE, null, foodCountName, null);
			...
			String choicePickUp = (String)JOptionPane.showInputDialog(framePickUp,		"Items:\n", "Pick Up Selection",		JOptionPane.PLAIN_MESSAGE, null, itemsName, null);
			...
			String choiceDrink = (String)JOptionPane.showInputDialog(frameDrink,		"Items:\n", "Drink Selection",		JOptionPane.PLAIN_MESSAGE, null, drinkOrNotName, null);
	...
	public int main()
	{
		...
		if(drinkOrNot == 0 && getInventoryCount(superDef1) > 0 && getLevel(1)+5 >= getCurrentLevel(1))
		{
			if(inCombat())
			{
				RunFromCombat();
		...
		if(getX() >= 581 && getX() <= 585 && getY() >= 3551 && getY() <= 3576)
		{
			...
			if(getInventoryCount(foodId) <= 1 || (getInventoryCount(mindRunes) <= cptRunesToBankAt && getInventoryCount(chaosRunes) <= cptRunesToBankAt))
			{
	...
	public void RunSouthOrAttackSouth(int[] npcIds)
	{
		...
		if(!npcFound)
		{
			if(random(1,8) == 5) //Try to Bypass invisible block
				...
				walkTo(getX(), getY()+1);
			else if(isReachable(getX()+1, getY()+1))
				walkTo(getX()+1, getY()+1);
			else if(isReachable(getX()-1, getY()+1))
				walkTo(getX()-1, getY()+1);
			else
				walkTo(getX()+random(-3, 3), getY()+random(0, 3));
