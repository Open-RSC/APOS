...
public class AA_BrimhavenGoldMiner extends AA_Script {
    ...
    private int bank() {
        if (Area.BANK.contains(this.playerX, this.playerY)) {
            ...
            if (this.getInventoryCount() == INITIAL_INVENTORY_SIZE) {
                if (this.getBankItemIdCount(ITEM_ID_COINS) < COIN_COUNT)
                {
    ...
    private int getBankItemIdCount(final int itemId)
    {
        ...
        {
            if (this.getBankId(index) == itemId)
            {
                count += this.getBankStack(index);
                break;
            }
        }
    ...
    private void statusReport() {
        ...
        System.out.printf("[%s] Time %s|Runtime %s|Mining L%d %s - %s xp/hr|Coins %d|Gold ore %s - %s ore/hr%n",
                this,
                ...
                this.getBankItemIdCount(ITEM_ID_COINS),
                this.oresMined,
