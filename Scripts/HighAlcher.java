...
public final class HighAlcher extends Script {
	...
	public HighAlcher(Extension ex) {
		super(ex);
	}
	...
	public int main() {
		...
		if (isBanking()) {
			...
			if (!hasInventoryItem(itm[ptr]) || getEmptySlots() > 1) {
				int w = getEmptySlots();
		...
		if (spell_failed) {
			if (getInventoryCount() < 30) {
				// open
				if (distanceTo(bankChest.x, bankChest.y) < 5) {
					atObject(bankChest.x, bankChest.y);
					return 1000;
				}
			...
			if (System.currentTimeMillis() - spell_fail_wait > 20000) {
				spell_failed = false;
			}
			return 500;
		}
	...
	public void onServerMessage(String str) {
		...
		if (str.contains("busy")) {
			menu_time = -1L;
		} else if (str.contains("the spell fails")) {
			spell_failed = true;
			prevent_logout = true;
			spell_fail_wait = System.currentTimeMillis();
			System.out.println("Spell failed! Banking for more stuff then waiting 20 secs");
		} else if (str.contains("you have been standing")) {
			prevent_logout = true;
		}
	}
